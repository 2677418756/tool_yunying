...
def dy_chuli(path,sku_path):
    ...
    # ①售后类型
    订单表 = pd.merge(订单表, 售后表[["订单编号", "售后类型"]], how='left', left_on='订单编号',
                           right_on='订单编号')
    订单表["售后类型"] = 订单表["售后类型"].fillna(0)
    # ①抖订【辰丰】
    # 发货时间
    ...
    # 团长
    订单表 = pd.merge(订单表, 团长表[["商品单号", "出单机构"]], how='left', left_on='商品单号',
                           right_on='商品单号')
    订单表["出单机构"] = 订单表["出单机构"].fillna("无")
    订单表.rename(columns={'出单机构': '团长'}, inplace=True)
