...
def 拼多多订单金额表(订单表):
    ...
    def 退货金额(flag):
        if flag == '已收货，退款成功' or flag == '已发货，退款成功':
            return 1
        else:
            return 0

    订单表['退款金额'] = 订单表['订单状态'].map(退款金额)
    订单表['退款金额'] = 订单表['退款金额'] * 订单表['商家实收金额(元)']

    订单表['退货金额'] = 订单表['订单状态'].map(退货金额)
    订单表['退货金额'] = 订单表['退货金额'] * 订单表['商家实收金额(元)']

    # 让订单创建日期显示正确
    订单表 = 订单表[~订单表.订单成交时间.isna()]
    ...
    # 建立订单金额透析表
    订单金额表 = pd.pivot_table(订单表, index=['订单成交时间'], values=['商家实收金额(元)', '退款金额', '退货金额'], aggfunc=[np.sum])
    # 去除表头sum
    ...
    # 出现0除0的风险，暂时以runtimewarning警告【具体体现在揽收退货率】
    订单金额表['退款率'] = 订单金额表.apply(lambda x: x['退款金额'] / x['商家实收金额(元)'], axis=1)
    订单金额表['销售退货率'] = 订单金额表.apply(lambda x: x['退货金额'] / x['商家实收金额(元)'], axis=1)
    订单金额表['揽收退货率'] = 订单金额表.apply(lambda x: x['退货金额'] / (x['商家实收金额(元)'] - x['退款金额']), axis=1)
    订单金额表['揽收退货率'] = 订单金额表['揽收退货率'].fillna(0)
...
def 天猫订单金额表(订单表):
    ...
    # #——————————————————————————————————————————————————————————————————————————————————样式调整
    return 订单金额表
def 视频号订单金额表(订单表,售后表,仓运单表):
    订单表["订单实际支付金额"] = 订单表["订单实际支付金额"].astype(float)

    合并 = pd.merge(订单表, 售后表[["订单编号", "售后申请日期"]], how='left', left_on='订单号',right_on='订单编号')
    合并 = pd.merge(合并, 仓运单表[["订单编号", "发货日期"]], how='left', left_on='订单号',right_on='订单编号')

    合并_有售后申请 = 合并[~合并.售后申请日期.isna()]

    合并_有物流时间 = 合并_有售后申请[~合并_有售后申请.发货日期.isna()]
    合并_无物流时间 = 合并_有售后申请[合并_有售后申请.发货日期.isna()]

    处理_有物流时间 = pd.pivot_table(合并_有物流时间,index='订单创建日期',values='订单实际支付金额',aggfunc = np.sum)
    处理_无物流时间 = pd.pivot_table(合并_无物流时间,index='订单创建日期',values='订单实际支付金额',aggfunc = np.sum)
    处理_有物流时间 = 处理_有物流时间.reset_index()
    处理_无物流时间 = 处理_无物流时间.reset_index()
    处理_有物流时间.rename(columns={'订单实际支付金额': '退货金额'}, inplace=True)
    处理_无物流时间.rename(columns={'订单实际支付金额': '退款金额'}, inplace=True)

    data = pd.pivot_table(订单表,index='订单创建日期',values='订单实际支付金额',aggfunc = np.sum)
    data = data.reset_index()

    data = pd.merge(data, 处理_有物流时间[["订单创建日期", "退货金额"]], how='left', left_on='订单创建日期',right_on='订单创建日期')
    data = pd.merge(data, 处理_无物流时间[["订单创建日期", "退款金额"]], how='left', left_on='订单创建日期',right_on='订单创建日期')
    data = data.fillna(0)

    data["退货率"] = data["退款金额"] / data["订单实际支付金额"]
    data["销售退货率"] = data["退货金额"] / data["订单实际支付金额"]
    data["揽收退货率"] = data["退货金额"]/ (data["订单实际支付金额"] - data["退款金额"])
    data = data.set_index("订单创建日期")
...
class Window(QWidget):
    ...
    def handel(self):
        ...
        需打开表格名列表 = ['订单表','售后表','仓运单表'] #成本表需自己选择
        # 实例化对象,调用方法
