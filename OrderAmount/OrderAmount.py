...
def 拼多多订单金额表(订单表):
    ...
    订单表['退款金额'] = 订单表['退款金额'] * 订单表['商品总价(元)']

    订单表['退货金额'] = 订单表['订单状态'].map(退货金额)
    订单表['退货金额'] = 订单表['退货金额'] * 订单表['商品总价(元)']

    # 让订单创建日期显示正确
    订单表 = 订单表[~订单表.订单成交时间.isna()]

    订单表.to_excel("D:/test.xlsx")
    ...
    # 建立订单金额透析表
    订单金额表 = pd.pivot_table(订单表, index=['订单成交时间'], values=['商品总价(元)', '退款金额', '退货金额'], aggfunc=[np.sum])
    # 去除表头sum
    ...
    # 出现0除0的风险，暂时以runtimewarning警告【具体体现在揽收退货率】
    订单金额表['退款率'] = 订单金额表.apply(lambda x: x['退款金额'] / x['商品总价(元)'], axis=1)
    订单金额表['销售退货率'] = 订单金额表.apply(lambda x: x['退货金额'] / x['商品总价(元)'], axis=1)
    订单金额表['揽收退货率'] = 订单金额表.apply(lambda x: x['退货金额'] / (x['商品总价(元)'] - x['退款金额']), axis=1)
    订单金额表['揽收退货率'] = 订单金额表['揽收退货率'].fillna(0)
