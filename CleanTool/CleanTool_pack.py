...
def 抖音处理模块(文件所在位置, 文件类型, 表格类型):
    ...
    if 表格类型 == '订单表':
        ...
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置,
                                 usecols=['主订单编号', '子订单编号', '选购商品', '商品数量', '订单应付金额',
                                          '订单提交时间', '订单状态', '售后状态', '商家编码', '商品ID', '达人昵称',
                                          '达人ID', '商品单价'])
        elif 文件类型 == '.csv':
            try:
        ...
        # 判断文件类型，防止平台后期修改文件的保存格式
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置, usecols=['订单号', '商品单号', '售后状态', '售后类型', '退商品金额（元）',
                                                        '售后申请时间', '退款方式', '商品ID'])
        elif 文件类型 == '.csv':
            data = pd.read_csv(文件所在位置,
                               usecols=['订单号', '商品单号', '售后状态', '售后类型', '退商品金额（元）', '售后申请时间',
                                        '退款方式', '商品ID'])
        ...
        data = data[(data.售后状态.isin(['待买家退货']) | data.售后状态.isin(['待商家处理']) | data.售后状态.isin(
            ['待商家收货']) | data.售后状态.isin(['同意退款，退款成功']))]
        # 格式转换
    ...
    #     判断表格名 = True
        ...
        if 文件类型 == '.xlsx':
            ...
            try:
                data = pd.read_csv(文件所在位置, usecols=['结算金额', '结算时间', '下单时间', '子订单号', '订单号'],
                                   dtype={"结算金额": str})
            except(UnicodeDecodeError):
                data = pd.read_csv(文件所在位置, usecols=['结算金额', '结算时间', '下单时间', '子订单号', '订单号'],
                                   dtype={"结算金额": str}, encoding='GB18030')
        ...
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置,
                                 usecols=['动帐流水号', '动账金额', '动账时间', '下单时间', '动账方向', '备注',
                                          '子订单号', '订单号', '商品ID', '运费实付', '订单退款'])
        elif 文件类型 == '.csv':
            try:
                data = pd.read_csv(文件所在位置,
                                   usecols=['动帐流水号', '动账金额', '动账时间', '下单时间', '动账方向', '备注',
                ...
                data = pd.read_csv(文件所在位置,
                                   usecols=['动帐流水号', '动账金额', '动账时间', '下单时间', '动账方向', '备注',
                                            '子订单号', '订单号', '商品ID', '运费实付', '订单退款'], encoding='GB18030')
        ...
        data = data.loc[:,
               ['动帐流水号', '订单号', '子订单号', '下单日期', '动账日期', '动账摘要', '收支金额', '下单时间',
                '动账时间', '商品ID', '动账方向', '运费实付', '订单退款']]
        # 统一字段名称
        data.rename(columns={'下单时间': '订单创建时间'}, inplace=True)
        data.rename(columns={'下单日期': '订单创建日期'}, inplace=True)
...
def 快手处理模块(文件所在位置, 文件类型, 表格类型):
    ...
    if 表格类型 == '订单表':
        ...
        if 文件类型 == '.xlsx':
            try:
                data = pd.read_excel(文件所在位置,
                                     usecols=['订单号', '订单创建时间', '订单状态', '实付款', '预估推广佣金',
                                              '商品名称', '商品ID', '成交数量', '售后状态', 'SKU编码', 'CPS达人ID',
                                              'CPS达人昵称', '商品单价', '团长ID', '发货时间', '快递单号', '快递公司'])
                data['预估推广佣金'] = data['预估推广佣金'].astype(str)
                data['预估推广佣金'] = data['预估推广佣金'].astype('float')
                # data.rename(columns={'预估推广佣金':'订单佣金'},inplace = True)
            except(ValueError):
                data = pd.read_excel(文件所在位置,
                                     usecols=['订单号', '订单创建时间', '订单状态', '实付款', '商品名称', '商品ID',
                                              '成交数量', '售后状态', 'SKU编码', 'CPS达人ID', 'CPS达人昵称', '商品单价',
                                              '团长ID', '发货时间', '快递单号', '快递公司'])
            ...
            try:
                try:
                    ...
                    data = pd.read_csv(文件所在位置,
                                       usecols=['订单号', '订单创建时间', '订单状态', '实付款', '商品名称', '商品ID',
                                                '成交数量', '售后状态', 'SKU编码', 'CPS达人ID', 'CPS达人昵称',
                                                '商品单价', '团长ID', '发货时间', '快递单号', '快递公司'])
        ...
        try:
            if 文件类型 == '.xlsx':
                data = pd.read_excel(文件所在位置,
                                     usecols=['发货/揽收/末条中转/派件/待取件/签收时间', '订单编号', '物流单号'])
            elif 文件类型 == '.xls':
                data = pd.read_excel(文件所在位置,
                                     usecols=['发货/揽收/末条中转/派件/待取件/签收时间', '订单编号', '物流单号'])
            elif 文件类型 == '.csv':
    ...
    #     判断表格名 = True
        ...
        if 文件类型 == '.xlsx':
            try:
                data = pd.read_excel(文件所在位置,
                                     usecols=['订单编号', '售后状态', '售后类型', '售后申请时间', '退款金额'])
                data['售后申请日期'] = pd.to_datetime(data['售后申请时间'])
                data['售后申请日期'] = data['售后申请日期'].dt.date
            except(ValueError):
        ...
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置,
                                 usecols=['订单号', '实际结算时间', '订单创建时间', "合计收入(元)", "合计支出(元)"])
        elif 文件类型 == '.csv':
            try:
                data = pd.read_csv(文件所在位置,
                                   usecols=['订单号', '实际结算时间', '订单创建时间', "合计收入(元)", "合计支出(元)"])
...
def 京东处理模块(文件所在位置, 文件类型, 表格类型):
    ...
    if 表格类型 == '订单表':
        ...
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置,
                                 usecols=['订单号', '商品ID', '商品名称', '订购数量', '结算金额', '下单时间',
                                          '订单状态'])
        elif 文件类型 == '.csv':
            try:
                data = pd.read_csv(文件所在位置,
                ...
                data = pd.read_csv(文件所在位置,
                                   usecols=['订单号', '商品ID', '商品名称', '订购数量', '结算金额', '下单时间',
                                            '订单状态'], encoding='GB18030')

        # 筛选去除不重要的数据
        data = data[~(data.订单状态.isin(['(删除)延迟付款确认']) | data.订单状态.isin(
            ['(删除)等待付款确认']) | data.订单状态.isin(['调度中']))]
        ...
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置,
                                 usecols=['订单编号', '费用结算时间', '金额', '单据类型', '费用项', '收支方向'])
        elif 文件类型 == '.csv':
            try:
                data = pd.read_csv(文件所在位置,
                                   usecols=['订单编号', '费用结算时间', '金额', '单据类型', '费用项', '收支方向'])
...
def 天猫处理模块(文件所在位置, 文件类型, 表格类型):
    ...
    if 表格类型 == '订单表':
        ...
        # 注意天猫的物流单号有空格
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置, usecols=['主订单编号', '子订单编号', '买家应付货款', '标题', '订单状态',
                                                        '订单创建时间', '物流单号', '物流公司', '购买数量', '退款状态',
                                                        '退款金额', '发货时间'])
        elif 文件类型 == '.csv':
            try:
                data = pd.read_csv(文件所在位置,
                                   usecols=['主订单编号', '子订单编号', '买家应付货款', '标题', '订单状态',
                                            '订单创建时间', '物流单号', '物流公司', '购买数量', '退款状态', '退款金额',
                                            '发货时间'])
            except(UnicodeDecodeError):
                data = pd.read_csv(文件所在位置,
                                   usecols=['主订单编号', '子订单编号', '买家应付货款', '标题', '订单状态',
                                            '订单创建时间', '物流单号', '物流公司', '购买数量', '退款状态', '退款金额',
                                            '发货时间'], encoding='GB18030')
        ...
        data = data.loc[:,
               ['业务基础订单号', '实际结算金额(元)', '入账时间', '实际结算日期', '备注', '账务类型', '业务描述']]
        ...
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置,
                                 usecols=['淘宝主订单号', '淘宝子订单号', '预估佣金', '商品名称', '商品ID', '达人昵称',
                                          '订单创建时间'])
        elif 文件类型 == '.csv':
            try:
                data = pd.read_csv(文件所在位置,
                                   usecols=['淘宝主订单号', '淘宝子订单号', '预估佣金', '商品名称', '商品ID',
                                            '达人昵称', '订单创建时间'])
            except(UnicodeDecodeError):
                data = pd.read_csv(文件所在位置,
                                   usecols=['淘宝主订单号', '淘宝子订单号', '预估佣金', '商品名称', '商品ID',
                                            '达人昵称', '订单创建时间'], encoding='GB18030')
        ...
        if 文件类型 == '.xlsx':
            data = pd.read_excel(文件所在位置,
                                 usecols=['主订单编号', '子订单编号', '商家编码', '订单状态', '订单创建时间',
                                          '购买数量', '买家应付货款', '退款状态', '标题'])
        elif 文件类型 == '.csv':
            try:
                data = pd.read_csv(文件所在位置,
                ...
                data = pd.read_csv(文件所在位置,
                                   usecols=['主订单编号', '子订单编号', '商家编码', '订单状态', '订单创建时间',
                                            '购买数量', '买家应付货款', '退款状态', '标题'], encoding='GB18030')
...
def 视频号处理模块(文件所在位置, 表格类型):
    ...
    if 表格类型 == '订单表':
        data = pd.read_excel(文件所在位置, usecols=['订单号', '下单时间', '订单状态', '订单实际支付金额', '订单运费',
                                          '支付方式', '支付时间', '交易单号', '物流公司', '快递单号',
                                          '带货人视频号', '预估佣金', '实际佣金', '实际服务费', '技术服务费',
                                          '技术服务费（将以人气卡形式返还）', '运费险预计投保费用',
                                          '商品编码(平台)', 'SKU编码(自定义)','商品名称', '商品属性', '商品价格', '商品数量', '商品发货',
                                          '商品售后', '商品已退款金额'], dtype=str)

        data = data.replace('-', '')
        data = data.replace('', 0)
        data["订单创建日期"] = data["下单时间"].apply(lambda x:str(x)[:10])
        data.rename(columns={'下单时间': '订单创建时间'}, inplace=True)
        data.rename(columns={'SKU编码(自定义)': '商品编码'}, inplace=True)
        data.rename(columns={'商品编码(平台)': '商品spu'}, inplace=True)

        data["订单实际支付金额"] = data["订单实际支付金额"].astype(float)
        data["订单运费"] = data["订单运费"].astype(float)
        data["订单实际支付金额"] = data["订单实际支付金额"].astype(float)
        data["预估佣金"] = data["预估佣金"].astype(float)
        data["实际佣金"] = data["实际佣金"].astype(float)
        data["实际服务费"] = data["实际服务费"].astype(float)
        data["技术服务费"] = data["技术服务费"].astype(float)
        data["技术服务费（将以人气卡形式返还）"] = data["技术服务费（将以人气卡形式返还）"].astype(float)
        data["运费险预计投保费用"] = data["运费险预计投保费用"].astype(float)
        data["商品价格"] = data["商品价格"].astype(float)
        data["商品数量"] = data["商品数量"].astype(float)
        data["商品已退款金额"] = data["商品已退款金额"].astype(float)

    elif 表格类型 == '订单流水表':
        data = pd.read_excel(文件所在位置,
                             usecols=["订单号", "下单时间", "结算状态", "实付款", "商家结算", "商家结算状态",
        ...
        data["达人"] = data["达人"].apply(
            lambda x: str(x).replace("运费险保费：", "").replace("，状态：", "").replace("运费险补缴：", ""))
        data["团长"] = data["团长"].apply(
            lambda x: str(x).replace("运费险保费：", "").replace("，状态：", "").replace("运费险补缴：", ""))
