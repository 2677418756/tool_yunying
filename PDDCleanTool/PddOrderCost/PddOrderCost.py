...
def 订单全状态表(path):
    ...
    合并数据['商品单号'] = 合并数据['订单编号']
    合并数据.rename(columns={'商品': '商家编码'}, inplace=True)
    合并数据.rename(columns={'商品规格': '商品名称'}, inplace=True)
    ...
    合并数据.rename(columns={'商品总价(元)': '商品单价'}, inplace=True)
    ...
    pd.DataFrame(订单全状态表).to_csv('aa.csv')
    print()
