...
def 抖音全订单状态表(union,账单表_处理, date):
    ...
    frame = frame.groupby('商品单号').apply(lambda x: x[x['售后申请时间'] == x['售后申请时间'].max()])
...
def 快手全订单状态表(union, date):
    ...
    frame["订单创建时间"] = pd.to_datetime(frame["订单创建时间"])
...
def 拼多多全订单状态表(path):
    ...
    合并数据['物流时间'] = 合并数据['物流时间'].fillna("2000-01-01 00:00:00")  # 缺失值替换
    合并数据['实际结算金额'] = 合并数据['实际结算金额'].fillna(0)  # 缺失值替换为0
    合并数据['订单应付金额'] = 合并数据['订单应付金额'].astype(float)  # 缺失值替换为0
    合并数据["预估推广佣金"] = 合并数据["预估推广佣金"].fillna(0)
    订单全状态表 = 合并数据.loc[:,
...
def 视频号全订单状态表(path):
    ...
    for 文件名 in 所有文件名列表:
        if '订单表' in 文件名:
            ...
            售后表 = CommonUtil().读取表格(path, 文件名);
            # print(bill_data[bill_data.columns[3]])
        elif '订单流水表' in 文件名:
            订单流水表 = CommonUtil().读取表格(path, 文件名);
        elif '仓运单表' in 文件名:
            仓运单表 = CommonUtil().读取表格(path, 文件名);
        else:
            continue

    合并 = pd.merge(订单表, 售后表[["订单编号", "退款状态", "售后申请时间"]], how='left', left_on='订单号',
                    right_on='订单编号')
    合并 = pd.merge(合并, 订单流水表[
        ["订单号", "达人佣金", "团长佣金", "商家结算时间", "实际结算金额", "达人名称", "团长名称"]], how='left',
                    left_on='订单号', right_on='订单号')
    ...
    订单状态表["回退状态"] = "已回"
    订单状态表["回退状态"] = np.where(
        (订单状态表["售后申请时间"] != "2000/01/01 00:00") & (订单状态表["实际结算时间"] == "2000-01-01 00:00"), "已退",
        订单状态表["回退状态"])
    订单状态表["回退状态"] = np.where(
        (订单状态表["售后申请时间"] != "2000/01/01 00:00") & (订单状态表["实际结算时间"] != "2000-01-01 00:00"), "回退",
        订单状态表["回退状态"])
    订单状态表["回退状态"] = np.where(
        (订单状态表["售后申请时间"] == "2000/01/01 00:00") & (订单状态表["实际结算时间"] == "2000-01-01 00:00"), "待回",
        订单状态表["回退状态"])
...
class Window(QWidget):
    ...
    def handel(self):
        ...
        if 平台类型 == '抖音':
            ...
            订单全状态表 = 视频号全订单状态表(self.清洗后文件夹路径)
            已揽收订单表 = pd.DataFrame();
        else:
