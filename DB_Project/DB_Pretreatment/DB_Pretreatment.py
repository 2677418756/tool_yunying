...
class Window(QWidget):
    ...
    def handel(self):
        ...
        if 平台类型 == '抖音':
            订单全状态表 = 订单全状态表.loc[:,['订单编号','商品单号','商家编码','商品名称','成交数量','订单应付金额','订单创建时间','订单状态','售后状态','售后申请时间','实际结算时间','实际结算金额','预估推广佣金','物流时间','回退状态','统计时间','达人昵称','出单机构','达人ID','商品单价']]
            已揽收订单表 = 已揽收订单表.loc[:,['订单编号','商品单号','商家编码','商品名称','成交数量','订单应付金额','订单创建时间','订单状态','售后状态','售后申请时间','实际结算时间','实际结算金额','预估推广佣金','物流时间','回退状态','统计时间','达人昵称','出单机构','达人ID','商品单价']]
        elif 平台类型 == '快手':
            订单全状态表['出单机构'] = 订单全状态表['CPS达人ID']
            已揽收订单表['出单机构'] = 已揽收订单表['CPS达人ID']
            订单全状态表 = 订单全状态表.loc[:, ['订单编号', '商品单号', '商品ID', '商品名称', '成交数量', '订单应付金额', '订单创建时间','订单状态', '售后状态', '售后申请时间', '实际结算时间', '实际结算金额', '预估推广佣金','物流时间', '回退状态', '统计时间', 'CPS达人昵称','出单机构','CPS达人ID','商品单价']]
            已揽收订单表 = 已揽收订单表.loc[:,['订单编号', '商品单号', '商品ID', '商品名称', '成交数量', '订单应付金额', '订单创建时间','订单状态', '售后状态', '售后申请时间', '实际结算时间', '实际结算金额', '预估推广佣金','物流时间', '回退状态', '统计时间', 'CPS达人昵称','出单机构','CPS达人ID','商品单价']]
