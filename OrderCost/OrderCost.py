...
def 视频号订单成本金额表(订单表, 售后表, 订单流水表):
    合并 = pd.merge(订单表[["订单号", "订单创建日期", "订单实际支付金额", "商品数量"]],
                    售后表[["订单编号", "退款金额"]], left_on='订单号', right_on='订单编号', how='left')
    合并 = pd.merge(合并, 订单流水表[["订单号", "达人佣金", "团长佣金"]], left_on='订单号', right_on='订单号',
                    how='left')
    合并 = 合并.fillna(0)
    合并[["订单实际支付金额", "退款金额", "商品数量", "达人佣金", "团长佣金"]] = 合并[
        ["订单实际支付金额", "退款金额", "商品数量", "达人佣金", "团长佣金"]].astype(float)
    合并["剩余销售金额"] = 合并["订单实际支付金额"] - 合并["退款金额"]
    处理 = pd.pivot_table(
        合并,
        index='订单创建日期',
        values=['剩余销售金额', '达人佣金', '团长佣金', '商品数量', '订单实际支付金额'],
        aggfunc={'剩余销售金额': np.sum, '达人佣金': np.sum, '团长佣金': np.sum, '商品数量': np.sum,
                 '订单实际支付金额': np.sum}
    )
    处理 = 处理.reset_index()
    处理["商品数量"] = 处理["商品数量"] * 6
...
class Window(QWidget):
    ...
    def handel(self):
        ...
        需打开表格名列表 = ['订单表','团长表','联盟表','淘客表',"账单表","推广表","售后表","仓运单表","订单流水表"] #成本表需自己选择
        # 实例化对象,调用方法
        ...
        if 平台类型 == '抖音':
            ...
            订单成本金额表 = 视频号订单成本金额表(订单表,售后表,订单流水表)
        else :
